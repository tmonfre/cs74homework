...
# perform a quantile transformation -- forces to uniform or normal distribution, spread out frequent values and reduces the impact of marginal outliers
# X = quantile_transform(X, axis=0)

# perform a quantile transformation on features 2,3,4,6
f2 = quantile_transform(X[:,1].reshape(-1,1))
f3 = quantile_transform(X[:,2].reshape(-1,1))
f4 = quantile_transform(X[:,3].reshape(-1,1))
f6 = quantile_transform(X[:,5].reshape(-1,1))
